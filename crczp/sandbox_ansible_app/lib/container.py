...
class BaseContainer(abc.ABC):
    ...
    @abc.abstractmethod
    def __init__(self, url, rev, stage, ssh_directory, inventory_path,
                 containers_path, credentials_path, cleanup=False):
...
class DockerContainer(BaseContainer):
    ...
    def __init__(self, url, rev, stage, ssh_directory, inventory_path,
                 containers_path, credentials_path, cleanup=False):
        """Initialize the container."""
        super().__init__(url, rev, stage, ssh_directory, inventory_path,
                         containers_path, credentials_path, cleanup)
...
class KubernetesContainer(BaseContainer):
    ...
    def delete_container(cls, container_name):
        ...
        try:
            ...
            raise CrczpException('Failed to delete job: {}'.format(exc))
