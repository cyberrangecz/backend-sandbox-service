...
class TestCreatePool:
    ...
    def test_create_pool_success(self, definition, created_by, get_terraform_client):
        pool = pools.create_pool(
            dict(definition_id=DEFINITION_ID, max_size=self.MAX_SIZE), created_by=created_by
        )
    ...
    def test_create_pool_invalid_definition(self, created_by):
        with pytest.raises(Http404):
            pools.create_pool(dict(definition_id=-1, max_size=self.MAX_SIZE), created_by=created_by)

    def test_create_pool_invalid_size(self, created_by):
        with pytest.raises(ValidationError):
            pools.create_pool(dict(definition_id=1, max_size=-10), created_by=created_by)
...
class TestPoolLock:
    ...
    def test_sandbox_get_and_lock_view_correct_token_successful(
    ...
    def test_sandbox_get_and_lock_view_incorrect_token_unsuccessful(
    ...
    def test_sandbox_get_and_lock_view_correct_token_full_pool(
